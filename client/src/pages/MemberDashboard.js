...
const PARASHA = {
  ...
  '2025-05-31':'נשא','2025-06-07':'נשא','2025-06-14':'בהעלותך',
  '2025-06-21':'שלח','2025-06-28':'קרח','2025-07-05':'חוקת',
  '2025-07-12':'בלק','2025-07-19':'פינחס','2025-07-26':'מטות-מסעי',
  '2025-08-02':'דברים','2025-08-09':'ואתחנן','2025-08-16':'עקב',
  '2025-08-23':'ראה','2025-08-30':'שופטים','2025-09-06':'כי תצא',
  '2025-09-13':'כי תבוא','2025-09-20':'נצבים-וילך',
...
function hebNewYear(y){
  const ny=hebElapsed(y),ny2=hebElapsed(y+1),dny=ny2-ny;
  let c=0;
  if(dny===356)c=2;
  else if(dny===382){if((ny-hebElapsed(y-1))===356)c=1;}
  return 347997+ny+c;
}

function hebMonthLens(y){
  const d=hebElapsed(y+1)-hebElapsed(y);
  const ny=d+(hebNewYear(y+1)-hebNewYear(y)-d);
  const diff=hebNewYear(y+1)-hebNewYear(y);
  if(diff===353)return[30,29,29,29,30,29,0,30,29,30,29,30,29];
  if(diff===354)return[30,29,30,29,30,29,0,30,29,30,29,30,29];
  if(diff===355)return[30,30,30,29,30,29,0,30,29,30,29,30,29];
  if(diff===383)return[30,29,29,29,30,30,29,30,29,30,29,30,29];
  if(diff===384)return[30,29,30,29,30,30,29,30,29,30,29,30,29];
  return[30,30,30,29,30,30,29,30,29,30,29,30,29];
}

function jdToHeb(jd){
  let year=Math.floor((jd-347997)*98496/35975351)+1;
  while(jd>=hebNewYear(year+1))year++;
  const lens=hebMonthLens(year);
  let rem=jd-hebNewYear(year);
  let month=7;
  while(rem>=(lens[month-1]||0)){rem-=(lens[month-1]||0);month=month===13?1:month+1;}
...
function MiniCalendar({selectedDate, onSelectDate, minDate, maxDate}){
  ...
  const mh=getHebDay(mid);
  const hebMonthStr=HEB_MONTH_NAMES[mh.month]||'';
    ...
    <div style={{border:'1px solid var(--clr-border)',borderRadius:'var(--radius-lg)',overflow:'hidden',marginBottom:'1rem',direction:'rtl'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',borderRight:'1px solid var(--clr-border)',borderTop:'1px solid var(--clr-border)'}}>
        ...
        {days.map((item,i)=>{
          ...
          return(
            <div key={i}
              onClick={()=>{if(!disabled&&cur)onSelectDate(date);}}
              style={{
                minHeight:58,padding:'4px 4px 3px',
                borderLeft:'1px solid var(--clr-border)',
                borderBottom:'1px solid var(--clr-border)',
                cursor:disabled?'not-allowed':'pointer',
                background:bg,
                outline:selected?'2px solid var(--navy-600)':'none',
                outlineOffset:selected?'-2px':'0',
                display:'flex',flexDirection:'column',alignItems:'flex-end',gap:1,
              }}>
              <div style={{display:'flex',justifyContent:'space-between',width:'100%',alignItems:'flex-start'}}>
                <span style={{fontSize:10,color:'var(--clr-text3)',lineHeight:1.4}}>{cur?hebNum:''}</span>
                <span style={{
                  fontSize:13,fontWeight:600,
                  color:disabled?'var(--clr-border2)':isSat?'var(--navy-700)':'var(--clr-text)',
                  background:isToday&&!selected?'var(--navy-700)':'transparent',
                  color:isToday&&!selected?'#fff':disabled?'var(--clr-border2)':isSat?'var(--navy-700)':'var(--clr-text)',
      ...
      <div style={{display:'flex',gap:10,padding:'6px 14px',background:'var(--clr-bg2)',borderTop:'1px solid var(--clr-border)',justifyContent:'flex-end',flexWrap:'wrap'}}>
        ...
        <span style={{fontSize:10,color:'var(--clr-text2)',display:'flex',alignItems:'center',gap:4}}><span style={{width:10,height:10,borderRadius:2,background:'#FFF8E6',border:'1px solid #FAE094',display:'inline-block'}}></span>חג</span>
      </div>
    </div>
  );
}
...
export default function MemberDashboard() {
  ...
  const handleSubmit = e => {
    ...
    submitMutation.mutate({
      ...
      shabbatLabel: PARASHA[key] || HOLIDAYS[key] || '',
      requestDate: key,
    });
  };
            ...
            <div className={styles.reqRow}>
              <div>
                ...
                <div className={styles.reqTime}>
                  {r.requestDate
                    ? new Date(r.requestDate+'T12:00:00').toLocaleDateString('he-IL',{dateStyle:'medium'})
